...
from markdown_blocks import markdown_to_html_node, quote_to_html_node, block_to_block_type
...
class TestInlineMarkdown(unittest.TestCase):
    ...
    def test_delim_multiple_different(self):
        node = TextNode("This is text with a **bolded** word and an *italicized* word", TextType.TEXT)
        new_nodes = split_nodes_delimiter([node], "**", TextType.BOLD)
    ...
    def test_split_no_links(self):
        node = TextNode(
            "This is a test with no link", TextType.TEXT,
        )
        new_nodes = split_nodes_link([node])
        self.assertListEqual(
            [
                TextNode("This is a test with no link", TextType.TEXT),
            ],
            new_nodes
        )
    ...
    def test_split_link_at_start(self):
        node = TextNode("[to boot dev](https://www.boot.dev) is a great place to learn backend", TextType.TEXT)
        new_nodes = split_nodes_link([node])
        self.assertListEqual(
            [
                TextNode("to boot dev", TextType.LINK, "https://www.boot.dev"),
                TextNode(" is a great place to learn backend", TextType.TEXT),
            ],
            new_nodes
        )
